...
class FinanceChart(MultiChart) :
    ...
    def addCandleStick(self, upColor, downColor) :
        ...
        if len(self.m_highData) > self.m_extraPoints :
            expectedWidth = int((self.m_totalWidth - self.m_leftMargin - self.m_rightMargin) / (len(
    ...
    def addParabolicSAR(self, accInitial, accIncrement, accMaximum, symbolType, symbolSize,
        ...
        ret = self.addLineIndicator2(self.m_mainChart, psar, fillColor, "")
        ret.setLineWidth(0)
    ...
    def addBand(self, upperLine, lowerLine, lineColor, fillColor, name) :
        ...
        layer = self.m_mainChart.addLineLayer2()
        layer.addDataSet(upperLine, lineColor, name)
        layer.addDataSet(lowerLine, lineColor)
        return self.m_mainChart.addInterLineLayer(layer.getLine(0), layer.getLine(1), fillColor)
    ...
    def addVolBars2(self, c, height, upColor, downColor, flatColor) :
        ...
        closeChange = ArrayMath(self.m_closeData).delta().replace(NoValue, 0).result()
    ...
    def addIndicator(self, height) :
        ...
        if self.m_legendFontColor != Transparent :
    ...
    def addADX(self, height, period, posColor, negColor, color) :
        ...
        posData = pos.result()
        negData = neg.result()
        if (len(posData) > 1) and (abs(posData[1] / NoValue - 1) > 1e-05) and (abs(negData[1
            ] / NoValue - 1) > 1e-05) :
        ...
        tr = ArrayMath(tr).expAvg(1.0 / period).result()
    ...
    #/ <returns>The XYChart object representing the chart created.</returns>
    def addCCI(self, height, period, color, deviation, upColor, downColor) :
        #typical price
        ...
        self.addThreshold(c, layer, deviation, upColor,  - deviation, downColor)
        return c
